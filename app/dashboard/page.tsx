  ...
  useEffect(() => {
    const mockOrder: Order = {
      ...
      telemetry: {
        ...
        uptime_seconds: 3660,
        gpu_metrics: [{ id: 'gpu-0', gpu_util_percent: 75, memory_util_percent: 45 }],
        container_metrics: { cpu_percent: 22, memory_percent: 31 },
        ports: [],
        workspace_url: '#'
      }
    };
    setOrders([mockOrder]);
    setStatus(`Displaying ${[mockOrder].length} active workspace(s).`);
  }, []);
...
  const handleAction = (orderId: string, action: "stop" | "restart" | "terminate") => {
