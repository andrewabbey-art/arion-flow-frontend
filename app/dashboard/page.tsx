...
type GpuMetric = { id: string; gpu_util_percent: number; memory_util_percent: number }
type ContainerMetrics = { cpu_percent: number | null; memory_percent: number | null }
type Port = { ip: string; is_ip_public: boolean; private_port: number; public_port: number; type: string }
type Telemetry = {
  pod_id: string
...
export default function DashboardPage() {
  ...
  // 🔐 check auth + authorized + org linkage
  useEffect(() => {
    async function checkAuth() {
      ...
      const { data: profile, error } = await supabase
        .from("profiles")
        .select("authorized, first_name, last_name")
        .eq("id", user.id)
        .single()
  ...
  // 📡 fetch telemetry
  useEffect(() => {
    if (!authorized || orders.length === 0) return
    ...
    const fetchTelemetry = async (orderId: string) => {
      try {
        ...
        if (json.ok) {
          ...
          setOrders((prev) =>
            prev.map((o) =>
              o.id === orderId ? { ...o, error: json.error ?? "Telemetry error" } : o
            )
          )
        }
      } catch {
